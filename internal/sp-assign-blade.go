	...
	if err = c.ConfigEstimateImpact(req, &lsServerPairs, &lsIssuesPairs); err == nil {
		...
		if lsServer != nil && lsIssues != nil {
			...
			if lsServer.ConfigState != "applied" {
				err = fmt.Errorf("configState=%s, configQualifier=%s, issues=%s", lsServer.ConfigState, lsServer.ConfigQualifier, strings.Join(issues, ", "))
			}
		}
	}
	if err == nil {
		...
		if err = c.ConfigConfMos(req, &out); err == nil {
			if lsBinding, err = SpGetBinding(c, spDn); err == nil {
				if lsBinding.OperState != "used" {
					err = fmt.Errorf("operState=%s, issues=%s", lsBinding.OperState, lsBinding.Issues)
				}
			}
		}
	}
	return lsBinding, fmt.Errorf("failed to assign blade: %w", err)
}
