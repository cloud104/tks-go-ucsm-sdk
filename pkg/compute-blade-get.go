	...
	if bladeSpec != nil {
		...
		if bladeSpec.Dn != "" {
			if matched, err = regexp.MatchString(wcardChars, bladeSpec.Dn); err != nil {
				return nil, fmt.Errorf("failed to match Dn regexp: %w", err)
			}
			if matched {
		...
		if bladeSpec.Model != "" {
			if matched, err = regexp.MatchString(wcardChars, bladeSpec.Model); err != nil {
				return nil, fmt.Errorf("failed to match model regexp: %w", err)
			}
			if matched {
				filter.Filters = append(filter.Filters, api.FilterWildcard{
					FilterProperty: api.FilterProperty{
		...
		rangeRegexp, err := regexp.Compile(`([0-9]+)\s*-\s*([0-9]+)`)
		if err != nil {
			return nil, fmt.Errorf("failed to compile range regexp: %w", err)
		}
